...
(defn -orn-schema []
  ...
  (reify IntoSchema
    ...
    (-into-schema [parent properties children options]
      ...
      (let [entry-parser (-entry-parser children {:naked-keys true} options)
         ...
         (reify
           ...
           (-validator [this]
             (let [validators (cond-> (mapv
                                      ...
                                      closed (into [(fn [m]
                                                      (reduce
                                                        (fn [acc k] (if (contains? (-keyset this) k) acc (reduced false)))
                                                        true (keys m)))]))
