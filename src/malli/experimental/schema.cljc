...
(defmacro -assert! [form & format-args]
  `(when-not ~form
     (m/-fail! ::error {:message (-format ~@format-args)})))

(c/defn -maybe-split-first [pred s]
  (if (pred (first s)) [(first s) (next s)] [nil s]))
...
(c/defn -process-fn-arity [env _ns _fn-name top-output-schema bind-meta [bind & body]]
  ...
  (when-let [bad-meta (seq (filter (or (meta bind) {}) [:tag :s? :s :schema]))]
    (m/-fail! ::error {:message "Meta not supported on bindings, put on fn name", :bad-meta (vector bad-meta)}))
